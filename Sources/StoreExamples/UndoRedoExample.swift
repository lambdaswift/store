            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        ScrollView {
                            VStack(alignment: .leading, spacing: 2) {
                                ForEach(Array(store.currentState.undoStack.enumerated().reversed()), id: \.offset) { index, text in
                                    Text("\(index + 1). \(text.prefix(30))\(text.count > 30 ? "..." : "")")
                                        .font(.caption)
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        ScrollView {
                            VStack(alignment: .leading, spacing: 2) {
                                ForEach(Array(store.currentState.redoStack.enumerated()), id: \.offset) { index, text in
                                    Text("\(index + 1). \(text.prefix(30))\(text.count > 30 ? "..." : "")")
                                        .font(.caption)
                                        .foregroundColor(.secondary)
                                }
